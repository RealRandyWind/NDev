...
namespace NDev
{
	...
	template<FSize SizeData, typename TypeValue, ESpace EnumSpace = ESpace::_Default>
	struct TVersor
	{
		...
		struct FSame
		{
			template<typename _TypeValue, ESpace _EnumSpace = ESpace::_Default>
			using Shape = TVersor<SizeData, _TypeValue, _EnumSpace>;

			template<typename _TypeValue, ESpace _EnumSpace = ESpace::_Default>
			using Size = TVersor<SizeData, _TypeValue, _EnumSpace>;

			template<FSize _SizeData, ESpace _EnumSpace = ESpace::_Default>
			using Type = TVersor<_SizeData, TypeValue, _EnumSpace>;

			template<FSize _SizeData, typename _TypeValue>
			using Space = TVersor<_SizeData, _TypeValue, EnumSpace>;
		};
		...
		operator TVersor<SizeLhs, TypeLhs>()
		{
			TVersor<SizeLhs, TypeLhs> Lhs;
			FSize Index, End;
		...
		TVersor<SizeData, TypeValue> & operator=(const TypeRhs &Rhs)
		{
			FSize Index, End;
		...
		TVersor<SizeData, TypeValue> & operator=(const TVersor<SizeRhs, TypRhs> &Rhs)
		{
			FSize Index, End;
		...
		static TVersor<SizeData, TypeValue> Like()
		{
			TVersor<SizeData, TypeValue> Result;
			return Result;
		}
